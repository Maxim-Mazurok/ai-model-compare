...
- **Task cost** uses Artificial Analysis's directly reported weighted-average USD cost per Intelligence Index task and reported time per task. Models for which Artificial Analysis does not report the cost field are omitted; the app does not synthesize missing costs or times from token counts, route prices, or route speed. This mode uses the Intelligence Index task cost for any selected score, so task profiles are hidden.
